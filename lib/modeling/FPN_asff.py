...
class fpn(nn.Module):
    ...
    def __init__(self, conv_body_func, fpn_level_info, P2only=False, panet_buttomup=False):
        ...
        # ---------------------------------------------------------------------------- #
        # Learning Spatial Fusion for Single-Shot Object Dection  __By RanJie
        # ---------------------------------------------------------------------------- #

        self.ASFF_level0 = ASFF(0)
        self.ASFF_level1 = ASFF(1)
        self.ASFF_level2 = ASFF(2)
        self.ASFF_level3 = ASFF(3)
    ...
    def forward(self, x):
        ...
        if self.panet_buttomup:
            ASFF_Feature_List = []
            ASFF_Feature_List.append(self.ASFF_level0(fpn_middle_blobs[0],fpn_middle_blobs[1],fpn_middle_blobs[2],fpn_middle_blobs[3]))
            ASFF_Feature_List.append(self.ASFF_level1(fpn_middle_blobs[0],fpn_middle_blobs[1],fpn_middle_blobs[2],fpn_middle_blobs[3]))
            ASFF_Feature_List.append(self.ASFF_level2(fpn_middle_blobs[0],fpn_middle_blobs[1],fpn_middle_blobs[2],fpn_middle_blobs[3]))
            ASFF_Feature_List.append(self.ASFF_level3(fpn_middle_blobs[0],fpn_middle_blobs[1],fpn_middle_blobs[2],fpn_middle_blobs[3]))

        # for i in range(len(ASFF_Feature_List)):
        #     print('ASFF_Feature_List_{}.shape: '.format(i), ASFF_Feature_List[i].shape)
        if self.panet_buttomup:
            ...
            for i in range(2, self.num_backbone_stages + 1):
                ...
                fpn_tmp = fpn_tmp + ASFF_Feature_List[self.num_backbone_stages - i]
                fpn_tmp = self.panet_buttomup_conv2_modules[i - 2](fpn_tmp)
                fpn_output_blobs.insert(0, fpn_tmp)
            fpn_output_blobs[-1] = self.panet_buttomup_conv2_modules[3](fpn_output_blobs[-1] + ASFF_Feature_List[-1])
...
class ASFF(nn.Module):
    def __init__(self, level, rfb=False, vis=False):
        ...
        if level == 0:
            self.stride_level_1 = nn.Conv2d(256, self.inter_dim, 3, 1, 1)
            self.stride_level_2 = nn.Conv2d(256, self.inter_dim, 3, 2, 1)
            self.stride_level_3 = nn.Conv2d(256, self.inter_dim, 3 ,2, 1)
            self.expand = nn.Conv2d(self.inter_dim, 256, 3, 1, 1)
        elif level == 1:
            #self.compress_level_0 = nn.Conv2d(256, self.inter_dim, 1, 1)

            self.stride_level_2 = nn.Conv2d(256, self.inter_dim, 3, 1, 1)
            self.stride_level_3 = nn.Conv2d(256, self.inter_dim, 3, 2, 1)
            self.expand = nn.Conv2d(self.inter_dim, 256, 3, 1, 1)
        elif level == 2:
            #self.compress_level_0 = nn.Conv2d(256, self.inter_dim, 1, 1)
            self.stride_level_3 = nn.Conv2d(256, self.inter_dim, 3, 1,1)
            self.expand = nn.Conv2d(self.inter_dim, 256, 3, 1,1)
        elif level == 3:
            self.expand = nn.Conv2d(self.inter_dim, 256, 3, 1,1)
        ...
        self.weight_level_3 = nn.Conv2d(self.inter_dim, compress_c, 1, 1)
    ...
    def forward(self, x_level_0, x_level_1, x_level_2,x_level_3):
        if self.level==0:
            ...
            level_1_resized_inter = F.max_pool2d(x_level_1, 3, stride=2, padding=1)
            level_1_resized = self.stride_level_1(level_1_resized_inter)

            level_2_downsampled_inter =F.max_pool2d(x_level_2, 3, stride=2, padding=1)
            level_2_resized = self.stride_level_2(level_2_downsampled_inter)

            level_3_downsampled = F.max_pool2d(x_level_3, 3 , stride=2 ,padding = 1)
            level_3_downsampled_inter = F.max_pool2d(level_3_downsampled, 3 , stride=2 ,padding = 1)
            level_3_resized = self.stride_level_3(level_3_downsampled_inter)

        elif self.level==1:
            #level_0_compressed = self.compress_level_0(x_level_0)
            level_0_resized =F.upsample(x_level_0, scale_factor=2, mode='nearest')
            level_1_resized =x_level_1
            #level_2_resized =self.stride_level_2(x_level_2)
            level_2_resized_inter = F.max_pool2d(x_level_2, 3, stride=2, padding=1)
            level_2_resized = self.stride_level_2(level_2_resized_inter)
            ...
            #level_0_compressed = self.compress_level_0(x_level_0)
            level_0_resized =F.upsample(x_level_0, scale_factor=4, mode='nearest')
            level_1_resized =F.upsample(x_level_1, scale_factor=2, mode='nearest')
            level_2_resized =x_level_2

            #level_3_resized = self.stride_level_3(x_level_3)
            level_3_resized_inter = F.max_pool2d(x_level_3, 3, stride=2, padding=1)
            level_3_resized = self.stride_level_3(level_3_resized_inter)
        elif self.level==3:
            level_0_resized = F.upsample(x_level_0,scale_factor=8,mode='nearest')
            level_1_resized = F.upsample(x_level_1, scale_factor=4, mode='nearest')
            level_2_resized = F.upsample(x_level_2, scale_factor=2, mode='nearest')
