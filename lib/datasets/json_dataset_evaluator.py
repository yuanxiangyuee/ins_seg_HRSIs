...
def _do_segmentation_eval(json_dataset, res_file, output_dir):
    ...
    _log_detection_eval_metrics(json_dataset, coco_eval, task_type='segm')
...
def _do_detection_eval(json_dataset, res_file, output_dir):
    ...
    coco_eval.evaluate()
    coco_eval.accumulate()
    _log_detection_eval_metrics(json_dataset, coco_eval, task_type='bbox')
...
def _log_detection_eval_metrics(json_dataset, coco_eval, task_type):
    ...
    logger.info('mean AP:  {:.1f}'.format(100 * ap_default))
    ...
    for cls_ind, cls in enumerate(json_dataset.classes):
        ...
        results_per_category.append(("{}".format(cls), float(ap * 100)))
        # tabulate it
